...
function SkillSection({ skill }: { skill: Skill }) {
  ...
  // pipeline-rendered range grid, floated into the title corner; the image
  // is generated at display size (integer cell pixels), never CSS-scaled
  const rangeLevelKey = `rangeImageUrlLevel${level}` as const;
  const rangeImg =
    (level > 1 ? (skill[rangeLevelKey] as string | null) : null) ??
    skill.rangeImageUrl;
