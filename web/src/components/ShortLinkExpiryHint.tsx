 ...
 * session-less row is swept after ANON_SHARE_DAYS. A link that quietly dies
 * three days after someone pastes it into Discord is worse than no short link
 * at all, so the expiry is stated up front — with the two ways to avoid it.
 *
 * Rendered only when logged out; a signed-in user's short link is permanent.
 */
import { ANON_SHARE_DAYS } from '../auth';

export function ShortLinkExpiryHint() {
  return (
    <p className="muted shortlink-hint">
      Short links expire after {ANON_SHARE_DAYS} days when you&apos;re not
