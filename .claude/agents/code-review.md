---
name: code-review
description: Cross-family POST-OP code review — PR-style review of a diff BEFORE it is committed/merged, for ordinary engineering work (features, refactors, fixes). The reviewer is always a different model family than the author — Claude-authored code is reviewed by kimi-code/k3 or qwen3.7-max via scripts/gates/dispatch-kimi.sh / dispatch-qwen.sh, Kimi/Qwen-authored code by claude-opus-5 via scripts/gates/dispatch-claude.sh; this native def (pinned to Opus) is the role body AND the same-family fallback. Sighted, findings-only, returns BLOCKED / FIX-BEFORE-MERGE / CLEAN as JSON.
...
4. **FIT.** Does it read like the code around it — naming, comment density, error-handling idiom,
   project conventions (`README.md`, `docs/frontend-conventions.md`, the skills under
   `.claude/skills/`)? New code should match the project's patterns, not import the author's
   defaults. And: does it reimplement something the repo already has?

   In this repo the load-bearing constraints live in `docs/frontend-conventions.md`, which is
   binding — check the diff against them specifically, since a violation is a BLOCKER and every one
   of them survives a skim: React 18 function components and hooks only, no global store; ALL
   styling in the single `web/src/styles.css` (no CSS modules, no Tailwind, no inline style objects
   beyond one-off computed values); path-based SPA routing through `router.ts` (`hrefFor` /
   `onSpaLinkClick` / `navigate` — never React Router, never hash routes, never a bare internal
   `<a>` that triggers a full reload); game data imported at build time from `data/*.json`, which is
   GENERATED by `npm run sync` and never hand-edited; game text rendered through `<RichText>` (it
   strips Tiptap HTML and resolves `[<kind>:<uuid>]` markers) and CDN art through `<GameIcon>` (local
   mirror with a remote fallback); pages loaded as `lazy()` route chunks so the dataset stays out of
   the entry chunk; and the ONE runtime API is Discord auth + saved profiles via `web/src/auth.ts`.
   The build codec in `src/share/buildCode.ts` is shared by the site, the bot and the infographics
